...
public class DragNDropListView2 extends ExpandableListView {
	...
	private DragNDropAdapter2 adapter;
        ...
        if(action == MotionEvent.ACTION_MOVE){
            ...
            if (Math.abs(deltaX) > HORIZONTAL_MIN_DISTANCE) {
                ...
                if (deltaX < 0) {
                    ...
                    if(adapter!=null){
                        adapter.showToast(mSwipeDetected);
                    }
                    //return true;
                }
                if (deltaX > 0) {
	...
	public void setAdapter(ExpandableListAdapter adapter) {
		...
		this.adapter = (DragNDropAdapter2) adapter;
	}
